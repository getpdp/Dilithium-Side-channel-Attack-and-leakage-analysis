...
class Dilithium:
    ...
    def _unpack_sk(self, sk_bytes):
        ...
        t0_len = 416 * self.k
        # print("Standard length:",3*32 + s1_len + s2_len + t0_len)
    ...
    def _unpack_sk_2544(self, sk_bytes):
        if self.eta == 2:
            s_bytes = 96
        else:
            s_bytes = 128
        ...
        s2_len = s_bytes * self.k
        t0_len = 416 * self.k
        print("Standard length:",2*32 + 48 +s1_len + s2_len + t0_len)
        if len(sk_bytes) != 2*32 + 48 + s1_len + s2_len + t0_len:
            raise ValueError("SK packed bytes is of the wrong length")
        
        # Split bytes between seeds and vectors
        sk_seed_bytes, sk_vec_bytes = sk_bytes[:64+48], sk_bytes[64+48:]
        
        # Unpack seed bytes
        rho, K, tr = sk_seed_bytes[:32], sk_seed_bytes[32:64], sk_seed_bytes[64:64+48]
